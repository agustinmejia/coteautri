        ...
        <div class="analytics-container">
            <div class="row">
                <div class="col-md-6" style="margin-bottom: 0px">
                    <h1><i class="voyager-pie-graph"></i> Finanzas</h1>
                </div>
                @if (auth()->user()->hasRole('admin'))
                <div class="col-md-6 text-right" style="margin-bottom: 0px">
                    <a href="{{ route('voyager.finances.create') }}" class="btn btn-success"><i class="voyager-plus"></i> Nuevo</a>
                </div>
                @endif
            ...
            <div class="row" style="text-align: center">
                @php
                    $pdf =  \App\Models\Finance::where('status',1)->get();
                @endphp
                @forelse ($pdf as $item)
                        ...
                        <article class="card">
                            @if(auth()->user()->hasRole('admin'))
                                <div style="position: absolute; top: 10px; right: 10px; z-index: 1">
                                    <a href="{{ route('voyager.finances.edit', $item->id) }}" class="btn btn-info" data-toggle="modal">
                                        <i class="fa-solid fa-edit"></i><span class="hidden-xs hidden-sm"><br></span>
                                    </a>
                                    <a href="#" class="btn btn-danger btn-delete" data-toggle="modal" data-target="#delete-modal" data-action="{{ route('voyager.finances.destroy', $item->id) }}">
